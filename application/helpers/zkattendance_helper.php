<?php
    ...
    function zkgetattendance($self) {
        ...
        if ( count($self->attendancedata) > 0 ) {
            ...
            while ( strlen($attendancedata) > 40 ) {
                ...
                // echo "ID: {$u1} -- CODE: ".substr($u[1], 4, 2) ."<br>";
                // echo "STATE: {$state} -- CODE: ".substr($u[1], 66, 2) ."<br>";
                // echo "timestamp: {$timestamp} -- CODE: ".substr($u[1], 58, 10) ."<hr>";
                ...
                $state = $state == 1 ? 'O' : 'I';
                array_push( $attendance, array( 'BADGENUMBER' => $id,
                                                'STATE' =>  $state,
                                                'CHECKTIME' => $timestamp ) );
    ...
    function zkcatpureimage($self)
    {
        
         $command = CMD_CAPTUREIMAGE;
        $command_string = '';
        $chksum = 0;
        $session_id = $self->session_id;
