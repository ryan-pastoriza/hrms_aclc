        ...
        <?php foreach ($options as $key => $value): ?>
          ...
          <?php else: $linkSegments = explode('/',$value['link']); ?>
            <li class="<?= $linkSegments[count($linkSegments) - 1] == $this->uri->segment($this->uri->total_segments()) ? 'active' : ''?>">
              ...
              <a href="<?= $value['link'] ?>"> <i class="<?= $linkSegments[count($linkSegments) - 1] == $this->uri->segment($this->uri->total_segments()) ? 'text-white' : 'text-blue'?> <?= $value['icon'] ?>"></i> <span><?= ucfirst($key) ?></span></a>
            </li>
          <?php endif ?>
        <?php endforeach ?>
      </ul>
