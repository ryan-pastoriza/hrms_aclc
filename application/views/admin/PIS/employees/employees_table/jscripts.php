 ...
 * @Last Modified time: 2019-08-01 10:47:42
 */
?>
<style>
	#submit{
	...
	$(document).on('click','#updateBTN',function(r){
	    ...
	    var newOptions = {
		        ...
		        success: function(e){
		                ...
		                $('.openEmpFileBtn[emp_id="'+globe_empID+'"]').trigger('click');
		                if (e.success == true) {
		                	reload_emp_list();
		                };  
...
$(document).on('click','.openEmpFileBtn',function(r){
	...
	//newley added -- july 30, 2019

	$.post("<?= base_url()?>index.php/admin/employees/company_info","emp_id="+emp_id,function(data){
		$("[name=company_name]").val("")
		$("[name=date_hired]").val("")
		$("[name=emp_status]").val("")
		$("[name=position]").val("")
		$("[name=section_program]").val("")
		$("[name=division]").val("")
		$("[name=immediate_supervisor]").val("")
		$("[name=department]").val("")
		$("[name=phic_no]").val("")
		$("[name=sss_no]").val("")
		$("[name=tin_no]").val("")
		...
		$("[name=company_name]").val(data.company)
		$("[name=date_hired]").val(data.date_hired)
		$("[name=emp_status]").val(data.emp_status)
		$("[name=position]").val(data.position)
		$("[name=section_program]").val(data.section_program)
		$("[name=division]").val(data.division)
		$("[name=immediate_supervisor]").val(data.immediate_sup)
		$("[name=department]").val(data.department)
		$("[name=phic_no]").val(data.phic_no)
		$("[name=sss_no]").val(data.sss_no)
		$("[name=tin_no]").val(data.tin_no)
		$("[name=pagibig_no]").val(data.pagibig_no)
		
	},'json')

	//end newley added -- july 30,2019
	...
	$.post("<?= base_url() ?>index.php/admin/employees/view_info","emp_id="+emp_id,	function(r){
		...
		$.each(r,function(k,v){
			...
			if(k == "employment_job_title"){

				if(v == "Student Assistant"){
					$('#studentCheckBox').attr("checked","checked").trigger("click").val("SA")
					...
					$('#employeeCheckBox').attr("checked","checked").trigger("click").val("employee")
					...
					$.each(v,function(key,obj){
						childrenDisp += "<tr style='width:100%;'>"+
											...
											"<td><span class='eaf_child_dob'  data-name='eaf_child_dob'  data-pk='"+obj.eaf_child_id+"'    data-value='"+obj.eaf_child_dob+"'>"  +obj.eaf_child_dob+"</span></td>"+
								  			"<td><div class='btn-group'>"+
											"<a href=# type='button' class='text-red dropdown-toggle' data-toggle='dropdown' aria-expanded='false' title='Delete'>"+
												"<span class='glyphicon glyphicon-trash'></span>"+
											"</a>"+
											"<ul class='dropdown-menu pull-right' role='menu'>"+
												"<li class='text-center'><span class='label alert-warning'>Are you sure?</span></li>"+
...
//gian author
$.post("<?= base_url() ?>index.php/admin/employees/ppst","emp_id="+emp_id,function(r){
	// console.log(r)
	$.each(r,function(k,v){
		$("[name="+k+"]").val(v)
	});
...
//@Author: ARIEL
	...
	educ2_tableOptions = {dom: '',
							ajax : "<?= base_url('index.php/admin/employees/get_educ2_json') ?>/"+emp_id,
							fnDrawCallback: function() {
								$('.ee_attainment').editable({
									mode:'inline',
									type: 'select',
...
//@Author: ARIEL
	...
	function clear_all() {
		...
		$('#parentSpouse .removeChild').trigger('click');
		$('#parentSpouse input').val('');
		$('#parentChildren .removeChild').trigger('click');
		$('#parentChildren input').val('');
		$('#parentRelative .removeChild').trigger('click');
		$('#parentRelative input').val('');
		$('#parentEducational .removeChild').trigger('click');
		$('#parentEducational input').val('');
		$('#bodyTeaching .removeChild').trigger('click');
		$('#bodyTeaching input').val('');
		$('#bodyCompanies .removeChild').trigger('click');
		$('#bodyCompanies input').val('');
		$('#bodyTrainingSeminar .removeChild').trigger('click');
		$('#bodyTrainingSeminar input').val('');
		$('#bodyOrganization .removeChild').trigger('click');
		$('#bodyOrganization input').val('');
		$('#bodyReference .removeChild').trigger('click');
		$('#bodyReference input').val('');
